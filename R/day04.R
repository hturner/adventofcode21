...
f04a <- function(x, boards) {
  for (i in seq_along(x)){
    for (b in seq_along(boards)){
      boards[[b]][boards[[b]] == x[i]] <- NA
      marks <- is.na(boards[[b]])
      win <- any(colSums(marks) == 5) || any(rowSums(marks) == 5)
      if (win) return(c(sum(boards[[b]], na.rm = TRUE), x[i]))
    }
  }
}
...
#' @rdname day04
#' @export
f04b <- function(x, boards) {
  won <- logical(length(boards))
  for (i in seq_along(x)){
    for (b in seq_along(boards)){
      if (won[b]) next
      boards[[b]][boards[[b]] == x[i]] <- NA
      ...
      if (win) {
        won[b] <- TRUE
        if (all(won)) return(c(sum(boards[[b]], na.rm = TRUE), x[i]))
      }
    }
  }
}
...
example_data_04 <- function(example = 1) {
  l <- list(
    a = list(c(7,4,9,5,11,17,23,2,0,14,21,24,10,16,13,6,15,25,12,22,18,20,8,19,3,26,1),
             list(matrix(c(22, 13, 17, 11,  0,
                           8,  2, 23,  4, 24,
                           21,  9, 14, 16,  7,
                           6, 10,  3, 18,  5,
                           1, 12, 20, 15, 19), byrow = TRUE, nrow = 5),
                  matrix(c(3, 15,  0,  2, 22,
                           9, 18, 13, 17,  5,
                           19,  8,  7, 25, 23,
                           20, 11, 10, 24,  4,
                           14, 21, 16, 12,  6), byrow = TRUE, nrow = 5),
                  matrix(c(14, 21, 17, 24,  4,
                           10, 16, 15,  9, 19,
                           18,  8, 23, 26, 20,
                           22, 11, 13,  6,  5,
                           2,  0, 12,  3,  7), byrow = TRUE, nrow = 5)))
    )
  l[[example]]
}
c
