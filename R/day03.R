...
f03a <- function(x) {
  ...
  nc <- ncol(x)
  ones <- colSums(x)
  gamma_binary <- as.integer(ones > nr/2)
  pwr <- 2^seq.int(nc - 1, 0)
  gamma <- gamma_binary %*% pwr
  epsilon <- (1 - gamma_binary) %*% pwr
  c(gamma = gamma, epsilon = epsilon)
}
...
#' @export
f03b <- function(x) {
  rating <- function(x, most_common){
    operator <- if (most_common) `>=` else `<`
    nc <- ncol(x)
    nr <- nrow(x)
    keep <- !logical(nr)
    for (i in seq(nc)){
      keep_bit <- as.integer(operator(sum(x[keep, i]), nr/2))
      keep <- keep & x[,i] == keep_bit
      nr <- sum(keep)
      if (nr == 1) break
    }
    pwr <- 2^seq.int(nc - 1, 0)
  ...
  c(oxygen_generator = rating(x, most_common = TRUE),
    CO2_scrubber = rating(x, most_common = FALSE))
}
